...
build_make_workdir
SRC_DIR="${BUILD_WORKDIR}/aardvark-dns"
PREFIX_DIR="${PODMAN_BUILD_PREFIX:-${BUILD_DIR}/podman-static-root}"
OUTPUT="${PREFIX_DIR}/usr/local/lib/podman/aardvark-dns"
...
export RUSTFLAGS="${AARDVARK_DNS_RUSTFLAGS:--C target-feature=+crt-static -C link-arg=-s}"
CARGO_BUILD_TARGET="${RUST_MUSL_TARGET}" cargo build --locked --release

mkdir -p "$(dirname "${OUTPUT}")"
cp "target/${RUST_MUSL_TARGET}/release/aardvark-dns" "${OUTPUT}"
chmod 0755 "${OUTPUT}"
build_strip_binary "${OUTPUT}"
build_verify_static_elf "${OUTPUT}"
"${OUTPUT}" --version >/dev/null
build_record_provenance "${PREFIX_DIR}" aardvark-dns "${AARDVARK_DNS_REPO}" "${AARDVARK_DNS_VERSION}" "${BUILD_CHECKOUT_SHA}"
